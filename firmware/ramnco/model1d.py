...
def nco(phase,wavetable,interpd=None,width=12,finalwidth=12,ampdither=None,samples=16384):
  ...
  for i in range(samples):
    ...
    if interpd:
      ...
      delta = int(np.round(delta))

      if np.abs(delta) > maxdelta: maxdelta = np.abs(delta)

      ## Apply dither to delta
      if ampdither:
        if delta >= 0:
          delta = delta + np.random.randint(*ampdither)
        else:
          delta = delta + np.random.randint(*ampdither)
...
npa = nco(p4,wavetable,interpd,width=width,finalwidth=12,ampdither=ampdither,samples=16384)
